	 ...
	 if( validaCampoVazio() ){	
		 $.ajax({
				...
				success: function(lista){
	  				...
	  				var tituloPrincipal = '';
	  				var textoPrincipal  = '';
	  				var nomeModal       = 'ModalRecursoEdit';
	  				var iconi           = 'success';
				...
				MensagemConfimacaoModal( "error", 'Erro na Adição/Atualização de Recurso', xhr.responseText, 'ModalRecursoEdit' );
		 }); 
...
//   				$("#vlrRecurso" ).val(json.valor);
 ...
 //	 montaSelectFamiliaModal( 0 );  ==> TROCAR PELA FUNCAO MONTA INFO. 
     verRecursoContrato( idRecurso );
 ...
 function montaSelectFamiliaModal( idFamilia ) {
		...
		var urlAction = document.getElementById("formContrato").action;
		 $.ajax({
				...
				// alert('Erro carregar select Produto: ' + xhr.responseText);
				MensagemConfimacaoModal( "error", 'Erro ao montar Select Familia!', xhr.responseText, 'ModalRecursoEdit' );
		 }); 
		 
	}

 $('#ModalRecursoEdit').on('hidden.bs.modal', function (e) {
...
function verRecursoContrato( idRecurso ) {	 
    ...
    $.ajax({
			...
			success: function(lista){
				 ...
				 idFamiliaFlavors = json.id_familia_flavors;
...
//				 $("#vlrRecurso"               ).val( json.valor              );				 
			...
			// alert('Erro carregar select Produto: ' + xhr.responseText);
			MensagemConfimacaoModal( "error", 'Erro ao Montar Recurso para Editar!', xhr.responseText, 'ModalRecursoEdit' );
	}); 
...
function validaCampoVazio() {
	 ...
	 if( valueStatusRecurso === null || valueStatusRecurso === '' || valueStatusRecurso.trim() === '' ){         
		// alert('Favor informar o Status.');
		 MensagemConfimacaoModal( "info", 'Validação', 'Favor informar o Status!', 'ModalRecursoEdit' );
		 ...
		 MensagemConfimacaoModal( "info", 'Validação', 'Favor informar a Retenção de Backup!', 'ModalRecursoEdit' );
		...
		// alert('Favor informar o Tipo de Disco.');
		 MensagemConfimacaoModal( "info", 'Validação', 'Favor informar o Tipo de Disco!', 'ModalRecursoEdit' );
		 document.getElementById("id_tipo_disco").focus();
		 return false;		 
	 }else  if( valueSo === null || valueSo === '' || valueSo.trim() === '' ){
		// alert('Favor informar o SO.');
		 MensagemConfimacaoModal( "info", 'Validação', 'Favor informar o SO!', 'ModalRecursoEdit' );
		 ...
		 MensagemConfimacaoModal( "info", 'Validação', 'Favor informar o Ambiente!', 'ModalRecursoEdit' );
		 ...
		 MensagemConfimacaoModal( "info", 'Validação', 'Favor informar o Tipo Servico!', 'ModalRecursoEdit' );
		 ...
		 MensagemConfimacaoModal( "info", 'Validação', 'Favor informar a Familia Flavors!', 'ModalRecursoEdit' );
...
//  				$("#vlrRecurso" ).val( json.valor );
  ...
  //				$("#id_familia_flavors").html(option).show();  
  			...
  			MensagemConfimacaoModal( "error", 'Erro ao buscar valores para o campo Família Flavors!', xhr.responseText, 'ModalRecursoEdit' );
  		}); 
    });
});
